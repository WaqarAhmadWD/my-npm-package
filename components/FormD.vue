...
<template>
  <div :class="custom_class?.main
      ? custom_class?.main
      : 'mb-6 grid gap-6 lg:grid-cols-2 w-full'
    ">
    <div v-for="(field, index) in filter" :key="index" class="w-full">
      <label class="mb-2 block text-sm font-medium text-default-900 ml-2" :for="field.name">{{ field.label }}</label>

      <!-- Textarea Field -->
      <input v-if="field.type === 'textarea'" :id="field.name" v-model="formModel[field.name]" as="textarea"
        :placeholder="field.placeholder"
        class="rounded-lg border border-default-200 px-4 py-2.5 dark:bg-default-50 w-full" :class="field.class"
        :name="field.name" @input="onChange(index)" />

      <!-- Regular Select Field -->
      <select v-if="field.type === 'select' && field.type !== 'searchableSelect'"
        class="rounded-lg border border-default-200 px-4 py-2.5 dark:bg-default-50 w-full"
        v-model="formModel[field.name]" :placeholder="field.placeholder" :name="field.name" :class="field.class"
      ...
      <input v-if="
        ...
        field.type === 'date' ||
        field.type === 'number'
      " :id="field.name" v-model="formModel[field.name]" :type="field.type" :placeholder="field.placeholder"
        class="rounded-lg border border-default-200 px-4 py-2.5 dark:bg-default-50 w-full" :class="field.class"
        :name="field.name" @input="onChange(index)" />
      ...
      <div v-if="field.type === 'checkbox'" class="flex flex-col gap-[1vw]">
        <div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" class="sr-only peer" v-model="formModel[field.name]" :id="field.name"
              :name="field.name" :class="field.class" @input="onChange(index)" />
            <div
      ...
      <div
        ...
        <input :id="field.name" v-model="formModel[field.name]" :type="field.showPass ? 'text' : 'password'"
          :placeholder="field.placeholder"
          class="w-full h-full border-0 ring-0 outline-none focus:outline-none focus:ring-0 px-4 py-3 rounded-lg"
          :class="field.class" :name="field.name" @input="onChange(index)" />
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
          class="cursor-pointer bg-transparent mx-2" v-if="field.showPass" @click="togglePass(index)">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="20"
          height="20" viewBox="0 0 256 256" xml:space="preserve" class="cursor-pointer mx-2" v-if="!field.showPass"
  ...
  <div :class="custom_class?.buttonPosition
      ? custom_class?.buttonPosition
      : 'w-full flex justify-start items-center col-span-2'
    ">
    <button @click="submitForm" :class="custom_class?.button
        ? custom_class?.button
